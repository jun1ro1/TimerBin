...
tmByteFields J1ClockKit::roundTime( const time_t time, byte &val ) {
    ...
    int field = sizeof(data)/sizeof(byte) - 1;
    while (field >= 0 && data[ field ] == 0 ) {
        field--;
    }
    if (field < 0) {
        // data[] is all 0
    ...
    if (field == (int)tmHour && data[field] >= 24) {
        val = 1;
        return tmDay;
    }
    else {
        val = data[ field ];
        return (tmByteFields)field;
    }
}
